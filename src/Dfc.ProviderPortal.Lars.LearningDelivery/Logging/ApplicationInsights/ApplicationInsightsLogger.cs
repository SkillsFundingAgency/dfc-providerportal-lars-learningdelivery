...
namespace Dfc.ProviderPortal.Lars.LearningDelivery.Logging.ApplicationInsights
{
    internal class ApplicationInsightsLogger : ILogger
    {
        ...
        private void StartTelemetryIfFunctionInvocation(IDictionary<string, object> stateValues)
        {
            ...
            {
                string functionName =  stateValues.GetValueOrDefault(ScopeKeys.FunctionName)?.ToString(); 
                string functionInvocationId = stateValues.GetValueOrDefault(ScopeKeys.FunctionInvocationId)?.ToString();
                string eventName = stateValues.GetValueOrDefault(ScopeKeys.Event)?.ToString();
